...
-- Tabla de relación muchos a muchos entre Procesos y Servicios
CREATE TABLE `ProcesosServicios` (
    `IDProcesoServicio` INT PRIMARY KEY AUTO_INCREMENT,
    `IDProceso` INT NOT NULL,
    `IDServicio` INT NOT NULL,
    CONSTRAINT `FK_Procesos_Servicios_Proceso` FOREIGN KEY (`IDProceso`) REFERENCES `Procesos`(`IDProceso`),
    CONSTRAINT `FK_Procesos_Servicios_Servicio` FOREIGN KEY (`IDServicio`) REFERENCES `Servicios`(`IDServicio`),
    UNIQUE KEY `UK_Proceso_Servicio` (`IDProceso`, `IDServicio`)
);
...
-- Tabla de relación muchos a muchos entre Servicios y Productos
CREATE TABLE `ServiciosProductos` (
    `IDServicioProducto` INT PRIMARY KEY AUTO_INCREMENT,
    `IDServicio` INT NOT NULL,
    `IDProducto` INT NOT NULL,
    CONSTRAINT `FK_Servicios_Productos_Servicio` FOREIGN KEY (`IDServicio`) REFERENCES `Servicios`(`IDServicio`),
    CONSTRAINT `FK_Servicios_Productos_Producto` FOREIGN KEY (`IDProducto`) REFERENCES `Productos`(`IDProducto`),
    UNIQUE KEY `UK_Servicio_Producto` (`IDServicio`, `IDProducto`)
);
